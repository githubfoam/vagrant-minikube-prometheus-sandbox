...
# vg-mk-pro01: Package 'docker.io' is not installed, so not removed
# vg-mk-pro01: E: Unable to locate package docker
# vg-mk-pro01: E: Unable to locate package docker-engine
...
kubectl apply -f /vagrant/monitoring-namespace.yaml

# Create configMap for prometheus.
kubectl apply -f /vagrant/prometheus-config.yaml

# Create prometheus deployment
kubectl apply -f /vagrant/prometheus-deployment.yaml

# Create prometheus service.
kubectl apply -f /vagrant/prometheus-service.yaml
...
kubectl apply -f /vagrant/grafana-service.yaml
...
kubectl apply -f /vagrant/node-exporter-daemonset.yaml
...
Vagrant.configure("2") do |config|
  ...
  config.vm.provider "virtualbox" do |vb|
    ...
    config.vm.define "vg-mk-pro-01" do |kalicluster|
      ...
      kalicluster.vm.hostname = "vg-mk-pro-01"
      ...
      kalicluster.vm.network "private_network", ip: "192.168.53.6"
    ...
    config.vm.define "vg-mk-pro-02" do |kalicluster|
      # https://app.vagrantup.com/ubuntu/boxes/hirsute64
      # kalicluster.vm.box = "ubuntu/hirsute64" #21.04
      # https://app.vagrantup.com/ubuntu/boxes/impish64
      # kalicluster.vm.box = "ubuntu/impish64" #21.10
      ...
      # https://app.vagrantup.com/ubuntu/boxes/xenial64
      # kalicluster.vm.box = "ubuntu/xenial64" #16.04
      kalicluster.vm.hostname = "vg-mk-pro-02"
      #bridged network,DHCP disabled, manual IP assignment                  
      # kalicluster.vm.network "public_network", ip: "10.10.8.67"
      #bridged network,DHCP enabled,auto IP assignment
      # kalicluster.vm.network "public_network"
      kalicluster.vm.network "private_network", ip: "192.168.53.7"
      # kalicluster.vm.network "forwarded_port", guest: 80, host: 81
      #Disabling the default /vagrant share can be done as follows:
      # kalicluster.vm.synced_folder ".", "/vagrant", disabled: true
      kalicluster.vm.provider "virtualbox" do |vb|
          vb.name = "vbox-mk-pro-02"
          vb.cpus = 2
          vb.memory = 4096          
          vb.gui = false     
      ...
      kalicluster.vm.provision "shell", inline: $ubuntu_docker_script
      kalicluster.vm.provision "shell", inline: $minikubescript, privileged: false, env: {"KUBERNETES_VERSION" => KUBERNETES_VERSION}
      kalicluster.vm.provision "shell", inline: $deploy_prometheus, privileged: false
    end
